...
func Recharge(referenceId string, customerId string, callerIp string) (err error) {
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		// quota 计算改用 decimal，避免浮点乘法误差，并拒绝溢出值。
		var quotaErr error
		quotaToAdd, quotaErr = common.QuotaFromDecimalStrict(
		...
		if err := creditTopUpQuota(tx, topUp.UserId, quotaToAdd, map[string]interface{}{"stripe_customer": customerId}); err != nil {
			return err
		}
	...
	if switchedGroup != "" {
		_ = RefreshUserGroupCache(topUp.UserId)
	}
...
func ManualCompleteTopUp(tradeNo string, callerIp string) error {
	...
	if switchedGroup != "" {
		_ = RefreshUserGroupCache(userId)
	}
	return nil
}
