...
type TaskPrivateData struct {
	...
	StorageArchived bool   `json:"storage_archived,omitempty"` // 结果已转存到用户个人存储桶,ResultURL 即桶地址,不再被上游 URL 覆盖
	// 计费上下文：用于异步退款/差额结算（轮询阶段读取）
	BillingSource  string              `json:"billing_source,omitempty"`  // "wallet" 或 "subscription"
	SubscriptionId int                 `json:"subscription_id,omitempty"` // 订阅 ID，用于订阅退款
	TokenId        int                 `json:"token_id,omitempty"`        // 令牌 ID，用于令牌额度退款
	ParentId       int                 `json:"parent_id,omitempty"`       // >>> jzlh-sub 子账号计费快照：任务退款/结算回主号钱包判据（0=非子号）
...
// GetByOnlyTaskId loads a task by its public task ID for internal background
// work that no longer has the originating user's request context.
