...
func TestInsertWithTxRejectsUnpreparedAffCode(t *testing.T) {
	setupUserUpdateTestState(t)

	user := User{
		Username: "oauth-user-without-prepare",
		Status:   common.UserStatusEnabled,
	}
	err := DB.Transaction(func(tx *gorm.DB) error {
		return user.InsertWithTx(tx, 0)
	})
	require.ErrorContains(t, err, "affiliate code is not prepared")
}
