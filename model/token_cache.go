...
// cacheDeleteToken is kept for callers that invalidate a token by its raw key.
// The mutation fence prevents an in-flight database reader from republishing a
// stale snapshot after the deletion.
func cacheDeleteToken(key string) error {
	return invalidateTokenCacheForMutation(key)
}

// cacheIncrTokenQuota updates an already hydrated token hash. It deliberately
// skips cold/incomplete hashes so a quota delta can never create a partial
// token cache that bypasses the normal database hydration path.
...
redis.call('HSET', KEYS[1], 'AccessedTime', ARGV[2])
return 1`
	_, err := common.RDB.Eval(context.Background(), script, []string{getTokenCacheKey(key)}, delta, common.GetTimestamp()).Int()
	return err
}
