...
func LogTaskConsumption(c *gin.Context, info *relaycommon.RelayInfo, taskArgs ...*model.Task) {
	...
	if otherRatios := info.PriceData.OtherRatios(); len(otherRatios) > 0 {
		other.SetPublic("task_ratios", otherRatios)
	}
	...
	if info.TaskRelayInfo != nil && info.TaskRelayInfo.PublicTaskID != "" {
		commissionSourceKey = model.BuildTaskCommissionSourceKey(
...
func taskBillingOther(task *model.Task) *model.LogOther {
	...
	if bc := task.PrivateData.BillingContext; bc != nil {
		...
		if bc.CompletionRatio != nil && *bc.CompletionRatio > 0 {
			other.SetPublic("completion_ratio", *bc.CompletionRatio)
		}
...
func RecalculateTaskQuota(ctx context.Context, task *model.Task, actualQuota int, reason string, clamps ...*common.QuotaClamp) {
	...
	model.RecordTaskBillingLog(model.RecordTaskBillingLogParams{
		...
		CommissionEventKey: model.TaskCommissionSettlementEvent(preConsumedQuota, actualQuota),
	})
}
