...
// ExcludeChannel removes a channel from subsequent selections for this request.
// It is used when the channel passed the optimistic concurrency filter but lost
// the atomic slot race before any upstream request was sent.
...
func CacheGetRandomSatisfiedChannel(param *RetryParam) (*model.Channel, string, error) {
	...
	if param.TokenGroup == "auto" {
		...
		for i := startGroupIndex; i < len(autoGroups); i++ {
			...
			selection := any(param.RequestPath)
			if len(filters) > 0 {
				selection = filters
			}
			channel, _ = model.GetRandomSatisfiedChannelExcluding(autoGroup, param.ModelName, priorityRetry, selection, param.excluded)
			if channel == nil {
		...
		channel, err = model.GetRandomSatisfiedChannelExcluding(param.TokenGroup, param.ModelName, param.GetRetry(), selection, param.excluded)
		if err != nil {
