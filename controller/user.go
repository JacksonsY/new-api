...
func buildSelfUserData(user *model.User) map[string]interface{} {
	...
	// jzlh-agent 宽限期判定失败按无宽限处理（fail-closed），不阻断 self/login/refresh 响应
	agentGraceAccess := false
	if user.AgentType == "" {
		var graceErr error
		agentGraceAccess, graceErr = model.HasAgentGraceAccess(user.Id, user.CommissionQuota)
		if graceErr != nil {
			common.SysError("failed to check agent grace access: " + graceErr.Error())
			agentGraceAccess = false
		}
	}
