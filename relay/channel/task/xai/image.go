...
func ExtractMultipartImageURL(c *gin.Context, info *relaycommon.RelayInfo) string {
	// gin dereferences c.Request inside MultipartForm(); guard it so a
	// non-multipart caller (EstimateBilling) degrades to "" instead of panicking.
	...
	// EstimateBilling only counts the image and passes a nil info; only the
	// request-building path carries a real RelayInfo whose action must flip.
	if info != nil {
