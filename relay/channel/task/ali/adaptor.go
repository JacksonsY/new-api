...
func (a *TaskAdaptor) convertToAliRequest(info *relaycommon.RelayInfo, req relaycommon.TaskSubmitReq) (*AliVideoRequest, error) {
	...
	// Metadata can overwrite Duration with zero. Keep the upstream request and
	// the billing multiplier aligned instead of allowing zero to be omitted from
	// the request while billing falls back to a smaller duration.
	if aliReq.Parameters.Duration <= 0 {
