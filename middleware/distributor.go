			...
			if shouldSelectChannel {
				...
				if preferredChannelID, found := service.GetPreferredChannelByAffinity(c, modelRequest.Model, usingGroup); found && channelhealth.ShouldEscapeAffinity(preferredChannelID) {
					...
					if err == nil && preferred != nil && preferred.Status == common.ChannelStatusEnabled {
						...
						affinitySatisfied = affinitySatisfied && channelSupportsRequestPath(preferred, c.Request.URL.Path, modelRequest.Model)
					}
				...
				if channel == nil {
					...
					if channel == nil {
						abortWithOpenAiMessage(c, modelUnavailableStatus(modelRequest.Model), noAvailableChannelMessage(c, usingGroup, modelRequest.Model), types.ErrorCodeModelNotFound)
						return
					}
				}
			}
