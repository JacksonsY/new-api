...
function leaveForOAuth(url: string) {
  ...
  // 只接受站内同源目标(上游 sanitizeAuthRedirect):防止回调侧跳到站外(开放重定向)。
  const safeRedirect = sanitizeAuthRedirect(redirect, window.location.origin)
  if (safeRedirect) {
    sessionStorage.setItem(OAUTH_REDIRECT_STORAGE_KEY, safeRedirect)
  } else {
