...
func StreamResponseOpenAI2Claude(openAIResponse *dto.ChatCompletionsStreamResponse, info convmeta.Meta) []*dto.ClaudeResponse {
	...
	if len(openAIResponse.Choices) == 0 {
		...
		if doneChunk || state.Done {
			...
			if oaiUsage == nil {
				// Some upstreams emit finish_reason before a final usage-only
				// chunk. Keep any tool-argument delta from this chunk, but defer
				// terminal events until usage arrives.
				return claudeResponses
			}
			appendStopOpenBlocks()
			claudeResponses = append(claudeResponses, &dto.ClaudeResponse{
